...
from .. import database,models,schemas,crud,oauth2
...
def query(id: int, db: Session = Depends(get_db),current_user:schemas.User=Depends(oauth2.get_current_user)):
...
@router.get('/')
def query(skip: int, limit: int, db: Session = Depends(get_db),current_user:schemas.User=Depends(oauth2.get_current_user)):
